...
import com.autohome.app.cars.service.common.ProtobufBaseComponent;
...
@RedisConfig(useCustomSlot = true)
@Slf4j
public class ProtobufSeriesDetailComponent extends ProtobufBaseComponent<SeriesDetailMessage> {
    ...
    public SeriesDetailMessage get(int seriesId) {
        return baseGet(makeParam(seriesId));
    }

    public CompletableFuture<SeriesDetailMessage> getAsync(int seriesId) {
        return baseGetAsync(makeParam(seriesId));
    }

    public CompletableFuture<List<SeriesDetailMessage>> getList(List<Integer> seriesId) {
        return baseGetListAsync(seriesId.stream().map(this::makeParam).collect(Collectors.toList()));
    }

    public List<SeriesDetailMessage> getListSync(List<Integer> seriesId) {
    ...
    public void refreshAll(Consumer<String> xxlLog) {
        ...
        allSeries.forEach(series -> {
            ...
            try {
                SeriesDetailMessage message = builder(
                        series,
                        fuelTypes,
                ...
                update(ParamBuilder.create(seriesIdParamName, series.getId()).build(), message);
                xxlLog.accept(series.getId() + " success");
                ThreadUtil.sleep(20);
            }catch (Exception e){
                log.error("执行失败",e);
            }
    ...
    public void refresh(int seriesId) {
        try {
            ...
            SeriesDetailMessage series = builder(entity,
            ...
            log.error(seriesId + "更新失败",e);
        }
    }

    private SeriesDetailMessage builder(SeriesEntity entity, List<Integer> fuelTypes, Integer seriesHasBookSpec, List<Integer> seriesIdsParamIsShow, List<Integer> allStopSeriesIsImageSpec, List<SpecEntity> specAll, List<VideoOrderModel> videoOrderList,CmsNewCarResult seriesOnTime) {
        SeriesDetailMessage.Builder dto = SeriesDetailMessage.newBuilder();
        ...
        dto.addAllDisplacementItems(getDisplacement(hasImgSpecAll, dto.getState()));
        ...
        tempSpecList.stream().min(Comparator.comparing(SpecEntity::getMinPrice)).ifPresent(spec -> dto.setMinPrice(spec.getMinPrice()));
        ...
        if (dto.getState()==10) {
            ...
            dto.setIsNewCar(!entityList.stream().anyMatch(x -> x.getState() != 10));
        }
        ...
        if (seriesAttentionDto != null && seriesAttentionDto.getSpecAttentions() != null && seriesAttentionDto.getSpecAttentions().size() > 0) {
            ...
            if(seriesAttentionDto.getSpecAttentions().get(0).getSpecname()!=null) {
        ...
        if (seriesOnTime!=null) {
            dto.setOnLineTime(DateUtil.format(seriesOnTime.getOnDate(),"yyyy-MM-dd HH:mm:ss"));
            dto.setNewBrandTagId(NumberUtils.toInt(String.valueOf(seriesOnTime.getTagId())));
        }
        if(StringUtils.isNotBlank(entity.getPriceDescription())) {
